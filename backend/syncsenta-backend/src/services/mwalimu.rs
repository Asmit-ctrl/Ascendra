...
///
/// Augments the prompt with mem0-recalled student context (when configured)
/// and persists the turn back to mem0 after a successful response so future
/// turns and MeTTa reasoning can build on it.
pub async fn route_request(
    ...
    // Recall prior context from mem0 (best-effort; no-op when unconfigured).
    let user_id = req.student_id.to_string();
    let recall_query = match req.input_type {
        InputType::Text => req.content.as_str(),
        _ => "recent learning context",
    };
    let recalled = match crate::services::memory::recall(
        &config.mem0_api_key,
        &user_id,
        recall_query,
        5,
    ...
    .await
    {
        Ok(mems) => mems,
        Err(e) => {
            tracing::warn!("mem0 recall failed (continuing without): {e}");
            Vec::new()
        }
    };

    // Build a request whose content carries the recalled context block.
    let augmented = if recalled.is_empty() {
        req.clone()
    } else {
        let prefix = crate::services::memory::format_for_prompt(&recalled);
        let mut a = req.clone();
        a.content = format!("{prefix}\nStudent question:\n{}", req.content);
        a
    };

    let response = match augmented.input_type {
        InputType::Image => route_to_gemini_vision(config, &augmented).await,
        InputType::Document => route_to_gemini_doc(config, &augmented).await,
        InputType::Voice => route_voice_pipeline(config, &augmented).await,
        InputType::Text => route_to_gpt4o(config, &augmented).await,
    }?;

    // Persist the turn (best-effort). Use the original (non-augmented) user
    // content so mem0 stores what the student actually said.
    if !user_id.is_empty() && req.student_id != Uuid::nil() {
        let messages = vec![
            crate::services::memory::Mem0Message {
                role: "user".into(),
        ...
        if let Err(e) = crate::services::memory::add_turn(
            &config.mem0_api_key,
            &user_id,
            messages,
            Some(metadata),
        ...
        .await
        {
            tracing::warn!("mem0 add_turn failed (non-fatal): {e}");
        }
    }
