...
/**
 * A lightweight, dashboard-wide way to ask Mwalimu about a concept without
  ...
  const sendMessage = async (rawMessage: string) => {
    ...
    try {
      ...
      if (!response.ok || !response.body) {
        let detail = `Mwalimu could not answer right now (${response.status}).`;
        try {
    ...
    <div className="fixed bottom-32 right-4 z-50 flex flex-col items-end gap-3 sm:bottom-6 sm:right-32">
      {open && (
        <section
          ...
          aria-label="Ask Mwalimu AI about a concept"
        ...
        >
          <header className="flex items-center gap-3 border-b bg-primary px-4 py-3 text-primary-foreground">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-semibold">Mwalimu AI</p>
            ...
            <Button
              ...
              onClick={closePanel}
              aria-label="Close Mwalimu AI chat"
            >
              <X className="h-5 w-5" />
            </Button>
          </header>
          ...
          >
            <div className="flex items-end gap-2">
              <Textarea
                ...
                aria-label="Question for Mwalimu AI"
              />
              {busy ? (
                <Button
                  type="button"
                  ...
                  aria-label="Stop Mwalimu's response"
                >
                ...
                <Button
                  ...
                  size="icon"
                  disabled={!input.trim()}
                  aria-label="Send question to Mwalimu AI"
                >
