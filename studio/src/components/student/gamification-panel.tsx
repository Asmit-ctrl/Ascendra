...
export type GamificationMode = 'mastery' | 'balanced' | 'achievement';

interface GamificationPanelProps {
  data: GamificationData;
  className?: string;
  /**
   * Controls how much extrinsic motivation the panel shows.
   * - 'mastery': hide points/rank, lead with badges-as-meaning and encouraging tone
   * - 'balanced' (default): level + streak + badges, no class rank
   * - 'achievement': everything, including rank and level progress
   */
...
export function GamificationPanel({
  ...
  const showRank = mode === 'achievement';
  const showPointsAndLevel = mode !== 'mastery';

  return (
    <div className={cn('space-y-4', className)}>
      {/* Hero card — content adapts to mode */}
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            {!showRank && !showPointsAndLevel && (
              <div className="text-center p-3 rounded-lg bg-purple-50 dark:bg-purple-950">
                <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">
                  {nextBadges.length}
                </p>
                <p className="text-xs text-purple-600 dark:text-purple-400">Badges to Go</p>
              </div>
            )}
          </div>
        </CardContent>
